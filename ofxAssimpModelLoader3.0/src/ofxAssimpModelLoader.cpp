...
bool ofxAssimpModelLoader::loadModel(ofBuffer & buffer, bool optimize, const char * extension){
	...
	aiSetImportPropertyInteger(props,AI_CONFIG_PP_PTV_NORMALIZE, true);
...
void ofxAssimpModelLoader::updateBones() {
	...
	for(unsigned int i=0; i<modelMeshes.size(); ++i) {
		...
		modelMeshes[i].animatedPos.assign(modelMeshes[i].animatedPos.size(),aiVector3D( 0, 0, 0 ));
		if(mesh->HasNormals()){
			modelMeshes[i].animatedNorm.assign(modelMeshes[i].animatedNorm.size(),aiVector3D( 0, 0, 0 ));
		}
...
//-------------------------------------------
void ofxAssimpModelLoader::getBoundingBoxForNode(const struct aiNode* nd,   aiVector3D* min,  aiVector3D* max, aiMatrix4x4* trafo)
{
    aiMatrix4x4 prev;
	unsigned int n = 0, t;
